...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    if (urlConfig) {
      try {
        ...
        // Auto-calculate if we have a valid configuration
        setTimeout(() => {
          handleCalculate();
        }, 100);
      } catch (error) {
  ...
  const handleCalculate = useCallback(async () => {
    try {
      ...
      const raidResults = calculateRaid({
        raidLevel: form.raidLevel,
        diskCount: form.diskCount,
        diskSize: form.diskSize,
        useMixedDisks: form.useMixedDisks,
        mixedDisks: form.mixedDisks || [],
      });

      // Calculate IOPS estimates
      const iopsResults = estimateIops({
        raidLevel: form.raidLevel,
        diskCount: form.diskCount,
        mediaType: form.mediaType,
        customRandomReadIops: form.customRandomReadIops,
        customSequentialReadMBps: form.customSequentialReadMBps,
      });
  ...
  const handlePresetSelect = useCallback((presetForm: CalculatorFormType) => {
    setForm(presetForm);
    ...
    setTimeout(async () => {
      try {
        ...
        const raidResults = calculateRaid({
          ...
          diskSize: presetForm.diskSize,
          useMixedDisks: presetForm.useMixedDisks,
          mixedDisks: presetForm.mixedDisks || [],
        });
        ...
        const iopsResults = estimateIops({
          raidLevel: presetForm.raidLevel,
          diskCount: presetForm.diskCount,
          mediaType: presetForm.mediaType,
          customRandomReadIops: presetForm.customRandomReadIops,
          customSequentialReadMBps: presetForm.customSequentialReadMBps,
        });
      ...
      <div className="container-custom">
        <div className="flex flex-col xl:flex-row gap-8">
          ...
          <div className="flex-1 min-w-0">
            ...
            {showResults && <AdBanner title="Sponsored" className="border-t border-white/10" />}
            ...
            <section id="how" className="section-padding">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {[
                  ...
                  >
                    <div className="mb-4">
                      <h3 className="text-xl font-bold text-primary mb-2">{raid.level}</h3>
                      <h4 className="text-lg font-semibold text-foreground mb-3">{raid.title}</h4>
                      <p className="text-sm text-muted-foreground">{raid.description}</p>
                    </div>
